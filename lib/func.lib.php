...
function activityAdd ($type, $iid, $stt, $to_uid) {
	global $u, $current, $member;
/*	if (!$stt) {
		if ($member['stt']) $stt = $member['stt'];
		else $stt = -1;
	}
*/	if (!$to_uid) $to_uid = $u;
	if ($iid) mysql_query("INSERT INTO `activity` (`type`, `stt`, `uid`, `to_uid`, `iid`, `time`, `last_updated`) VALUES ('{$type}', '{$stt}', '{$u}', '{$to_uid}', '{$iid}', '{$current}', '{$current}') ");
}
...
function rate ($type, $iid, $star, $title, $content) {
	global $u, $curint, $current;
	$table = $type.'_ratings';
////	$cInfo = getRecord($type.'^uid', "`id` = '{$iid}' ");
	$rated = 0;
	if (countRecord($table, "`uid` = '{$u}' AND `iid` = '{$iid}'") <= 0) {
		$rr = mysql_query("INSERT INTO `$table` (`uid`, `iid`, `rate`, `title`, `content`, `time`) VALUES ('$u', '$iid', '$star', '$title', '$content', '$current')");
		if ($rr) $rated = 1;
...
function sendNoti ($type, $iid, $to, $content) {
	...
	return mysql_query("INSERT INTO `notification` (`type`, `iid`, `uid`, `to_uid`, `content`, `time`) VALUES ('{$type}', '{$iid}', '{$u}', '{$to}', '{$content}', '{$curint}')");
//	addNoti($to);
...
function removeNoti ($type, $from, $to, $iid, $content) {
	mysql_query("DELETE FROM `notification` WHERE `type` = '{$type}' AND `uid` = '{$from}' AND `to_uid` = '{$to}' AND `iid` = '{$iid}' AND `content` = '{$content}' ");
//	subtractNoti($to);
}
