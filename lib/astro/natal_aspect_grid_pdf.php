...
for ($i = 0; $i <= $last_planet_num; $i++) {
	drawboldtext($im, 18, 0, $margins + $i * $cell_width, $cell_height * ($i + 1), $black, __ASTRO.'/HamburgSymbols.ttf', chr($pl_glyph[$i]), 0);

	// display planet data in the right-hand table
	drawboldtext($im, 16, 0, $margins + $left_margin_planet_table, $cell_height * ($i + 1), $black, __ASTRO.'/HamburgSymbols.ttf', chr($pl_glyph[$i]), 0);
	imagettftext($im, 10, 0, $margins + $left_margin_planet_table + $cell_width * 2, $cell_height * ($i + 1) - 3, $blue, __ASTRO.'/arial.ttf', $pl_name[$i]);
	$sign_num = floor($longitude[$i] / 30) + 1;
	drawboldtext($im, 14, 0, $margins + $left_margin_planet_table + $cell_width * 5, $cell_height * ($i + 1), $black, __ASTRO.'/HamburgSymbols.ttf', chr($sign_glyph[$sign_num]), 0);
	imagettftext($im, 10, 0, $margins + $left_margin_planet_table + $cell_width * 6, $cell_height * ($i + 1) - 3, $blue, __ASTRO.'/arial.ttf', Convert_Longitude($longitude[$i]) . " " . $rx1[$i]);
}
...
for ($i = 0; $i <= $last_planet_num - 1; $i++) {
	for ($j = $i + 1; $j <= $last_planet_num; $j++) {
		...
		if ($q > 0) drawboldtext($im, 14, 0, $margins + $cell_width * ($i + 0.15), $cell_height * ($j + 1 - 0.20), $asp_color[$q], __ASTRO.'/HamburgSymbols.ttf', chr($asp_glyph[$q]), 0);
	}
}
