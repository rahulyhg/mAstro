	<section id="aspect" class="hide">
		...
		<div class="chart-report-content">
			<div class="chart-report-des"><? echo astro('aspect')['content'] ?></div>
<?	for ($i = 0; $i <= 8; $i++) {
		for ($j = $i + 1; $j <= 9; $j++) {
...
*/ ?>
			<div class="paragraph" id="<? echo trendCode($aTitle) ?>" data-rid="<? echo astro(trendCode($aTitle))['id'] ?>" data-trans-num="<? echo countTrans(astro(trendCode($aTitle))['id']) ?>">
				<h3><b><? echo $lang[$pl_name[$i]].' '.$lang[$aspect].' '.$lang[$pl_name[$j]] ?></b></h3>
				<div class="chart-paragraph-content"><? echo astro(trendCode($aTitle))['content'] ?></div>
			</div>
<? 			}
		}
	} ?>
