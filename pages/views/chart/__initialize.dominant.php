<? 	unset($PATH,$out,$longitude1,$longitude,$declination,$row,$out);
	$inmonth = $month;
	$inday = $day;
	$inyear = $year;
	...
	exec (SWEPH."swetest -edir".SWEPH." -b$utdatenow -ut$utnow -p0123456789 -eswe -house$my_longitude,$my_latitude, -fPldj -g, -head", $out);
...
$chartRuler = $plRar[0];
// Dominant planet
$planet_power_sort = $planet_power;
rsort($planet_power_sort);
$dominant_planet_key = array_search($planet_power_sort[0], $planet_power);
$dominant_planet = $pl_name[$dominant_planet_key];
// Dominant sign
for ($xx = 1; $xx <= 12; $xx++) {
...
$sign_power_sort = $sign_power;
rsort($sign_power_sort);
$dominant_sign_key = array_search($sign_power_sort[0], $sign_power);
$dominant_sign = $sign_name[$dominant_sign_key];
// Domiant house
$house_power_sort = $house_power;
rsort($house_power_sort);
$dominant_house_key = array_search($house_power_sort[0], $house_power);
$dominant_house = $dominant_house_key;
