...
if (($bday1 != $bday2) || ($bhour1 != $bhour2) || ($timezone1 != $timezone2) || ($ns_txt1 != $ns_txt2) || ($ew_txt1 != $ew_txt2) || ($my_longitude1 != $my_longitude2) || ($my_latitude1 != $my_latitude2) ) $ok = true;
	...
	if ($ok == true) {
		...
		$cIn = getRecord('relationship', "`uid` = '{$u}' AND 
			...
			OR
				(`name` = '{$name_}' AND `gender` = '{$gender_}' AND `birthday` = '{$bday_}' AND `birthhour` = '{$bhour_}' AND `timezone` = '{$timezone_}' AND `country` = '{$country_}' AND `town` = '{$town_}' AND `long_deg` = '{$long_deg_}' AND `lat_deg` = '{$lat_deg_}' AND `long_min` = '{$long_min_}' AND `lat_min` = '{$lat_min_}' AND `ew` = '{$ew_txt_}' AND `ns` = '{$ns_txt_}' )
			");
...
*/
		if ($cIn['id']) echo $rLink.'/'.$cIn['uname'].'/'.$cIn['n'];
		else {
			$nn = countRecord($tb, "`uid` = '{$u}' ") + 1;
			$ins = insert($tb, "`name`, `aid`, `uid`, `uname`, `n`, `gender`, `birthday`, `birthhour`, `timezone`, `long_deg`, `long_min`, `lat_deg`, `lat_min`, `ew`, `ns`, `country`, `town`, `time`, `last_updated`", " '{$name}', '{$aID}', '{$u}', '{$member['username']}', '{$nn}', '{$gender}', '{$bday}', '{$bhour}', '{$timezone}', '{$long_deg}', '{$long_min}', '{$lat_deg}', '{$lat_min}', '{$ew_txt}', '{$ns_txt}', '{$country}', '{$town}', '{$current}', '{$current}' ");
			$cIn = getRecord($tb, "`uid` = '{$u}' AND `n` = '{$nn}' ");
			activityAdd($tb, $cIn['id'], 0);
			if ($ins) echo $rLink.'/'.$member['username'].'/'.$nn;
		}
	}
} ?>
