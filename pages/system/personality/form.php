...
//		$resultAr[$ao] = countType($ao);
	...
	$check = getRecord('mbti', "`uid` = '{$u}' AND `uname` = '{$member['username']}' AND `mbti` = '{$result}' AND `mbti_data` = '{$res}' ");
	if ($check['id']) echo $pLink.'/u/'.$check['uname'].'/'.$check['n'];
	else {
		$ins = insert('mbti', "`uid`, `uname`, `n`, `full`, `gender`, `mbti`, `mbti_data`, `time`", " '{$u}', '{$member['username']}', '{$nn}', '{$full}', '{$gender}', '{$result}', '{$res}', '{$current}' ");
		if ($ins) {
			$check = getRecord('mbti', "`uid` = '{$u}' AND `uname` = '{$member['username']}' AND `mbti` = '{$result}' AND `mbti_data` = '{$res}' ");
