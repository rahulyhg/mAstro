...
$(function () {
	$.getScript(PLUGINS + '/meditor/caret/dist/jquery.caret.min.js')
});

(function ($) {
	$.fn.meditor = function (options) {
		var settings = $.extend({
			fonts: ["Lato", "Arial", "Comic Sans MS", "Courier New", "Monotype Corsiva", "Papyrus"],
			headings: [
				["p", "Normal"],
				["h1", "Heading 1"],
				["h2", "Heading 2"],
		...
		return this.each(function (e) {
			var $this = $(this).addClass('hidden-textarea');
			var firstVal = $this.val();
			var textareaID;
			if ($this.attr('id')) textareaID = $this.attr('id');
			else textareaID = '';
			heit = $this.attr('style').split('height:')[1].split('px')[0];
			var containerDiv = $("<div/>", {
				id: textareaID,
				class: 'meditor-container sceditor-container form-controls',
				style: 'min-height:' + heit + 'px'
			});
...
//			if ($this.attr('name') != 'cmt-content') containerDiv.css('height', 120);
			...
			containerDiv.resizable({
				...
				stop: function (event, ui) {
					...
					containerDiv.find('.meditor-iframe').height(ui.size.height)
				}
			});
			$editorBody.on("keyup", function (evt) {
				var exc = [8, 17, 33, 34, 35, 36, 37, 38, 39, 40];
				$this.val($(this).html());
//				alert(evt.keyCode);
/*				if (!evt.ctrlKey && evt.which != 9 && $.inArray(evt.keyCode, exc) == -1) {
					caretPos = $(this).caret('pos');
					$this.val($(this).html()).focus();
...
*/				var start = /@/ig; // @ Match
				...
				if (go.length > 0 && go != null) {
					topp = parseInt($(this).caret('position').top) + 25;
					if ($this.is('.full-me')) topp += 40;
//					offSet = $editorBody.offset();
//					var topp = 20;
					if (!containerDiv.find('.select-tag-users').length) $(this).after('<div class="select-tag-users" style="position: absolute; top: ' + topp + 'px"><a class="select-tag-close fa fa-times"></a> <div class="content"><div class="spinner"> <div></div> <div></div> <div></div> </div></div></div>');
					containerDiv.find('.select-tag-users').slideDown('show').find('.select-tag-close').click(function () {
						$(this).closest('.select-tag-users').remove()
					});
					containerDiv.find('.select-tag-users .content').html("Tag someone...");
					if (nameSpace != null) containerDiv.find('.select-tag-users').remove();
					else if (name.length > 0) {
						$.ajax({
							type: "POST",
							url: MAIN_URL + "/pages/friendList.php",
							data: dataString,
							cache: false,
							success: function(data) {
//								containerDiv.find('.select-tag-users').slideDown('show').focus().attr('tabindex', 1);
								containerDiv.find('.select-tag-users').find('.content').html(data).show();
								containerDiv.find(".select-tag-users .one-fr-search").on("click", function() {
									...
									ebody.html(content + '<span>&nbsp;</span><span class="tag-name" contenteditable="false">+' + username + ' </span><span>&nbsp;</span>');
									containerDiv.find('.select-tag-users').remove();
								});
							}
						});
					}
				} else $('.select-tag-users').remove()
			...
			var btnUnderline = $("<a/>", {
				href: "#",
				text : "Li",
				data : { commandName : "InsertUnorderedList" },
				click : execCommand 
			}).appendTo(buttonPane);
			...
			$.each(settings.headings, function (i,v) {
				$("<option/>",{
					value : v[0],
					text : v[1]
				}).appendTo(selectHeading);
			});
			var tbImg = $("<div/>", {
				class : 'meditor-dropdown meditor-insert-img hide',
				html : '<input class="col-lg-9" id="img_url" type="text"/>'
			}).appendTo(containerDiv);
			tbImg.hide();
			var btnImg = $("<a/>", {
				text : "Img"
			}).appendTo(buttonPane).on('click', function () {
				tbImg.toggle()
			});
			var goImg = $("<a/>", {
				href: "#",
				id: "go",
				text : "Go",
				data : { commandName : "insertimage" },
				click: execCommand
			}).appendTo(tbImg).after(' <div class="clearfix"/>');
			function execCommand (e) {
				...
				document.execCommand($(this).data("commandName"), false, this.value || $('#img_url').val() || "");
				$('#img_url').val('');
				$('.meditor-dropdown').hide()
				return false
			}
			}
		})
