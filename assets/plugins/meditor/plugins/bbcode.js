...
(function($, window, document) {
	...
	// SPECIAL FA
	$.sceditor.FASpecialCharacters = [";", " ", "\"", "<", ">",
			"%", "|", "\\",
			"^", "~", "[", "]", "`"];
	$.sceditor.FAEncodedCharacters = ["%3B", "%20", "%22", "%3C", "%3E",
			"%25", "%7C", "%5C",
			"%5E", "%7E", "%5B", "%5D", "%60"];
		
	/**
	 * Encode comme encodeURI,
	 * @param {type} url
	 * @returns {url|string}
	 */
	
	/**
	 * Decode comme decodeURI, mais sans decoder les accents
	 * 
	 * @param {string} url l'URL Ã¯Â¿Â½ encoder
	 * @returns {string} l'URL encodÃ¯Â¿Â½e
	 */
	$.sceditor.FAdecodeURI = function(url) {
		for(var i = 0; i < $.sceditor.FAEncodedCharacters.length; i++) {
			url = url.replace(new RegExp($.sceditor.FAEncodedCharacters[i], 'g'), $.sceditor.FASpecialCharacters[i]);
		}
		return url;
	};
	
	$.sceditor.ShowHideToolbarElements = function() {
		if($(".sceditor-button-subscript").parent().css("display") !== "none")
		{
		...
		{
			$(".sceditor-button-subscript").parent().css("display", "inline-block");
			$(".sceditor-button-fascroll").parent().css("display", "inline-block");
		}
	};
	// FIN
	...
	$.sceditor.BBCodeParser = function(options) {
		...
		tokenizeTag = function(type, val) {
			...
			if(type === "open" && (matches = val.match(/\[([^\]\s=\(]+)(?:([^\]]+))?\]/)))	// Ajout de la parenthese suivant le token
			{
			...
			while(toks.length > 0)
			{
				...
				else if(token.type === tokenType.open)
				{
					...
					if(token.attrs)
					{
						...
						{
							if(token.attrs.defaultattr.charAt(0) === "(" && token.attrs.defaultattr.charAt(token.attrs.defaultattr.length-1) === ")") // Adaptation FA
	...
	$.sceditor.plugins.bbcode = function() {
		...
		mergeSourceModeCommands = function(editor) {
			var merge = {
						...
						function(values) {
							if(values['src'] && values['src'] !== "http://")
							{
								...
								if(values['width'] && !values['height'])
									dim = '(' + values['width'] + 'px,' + values['width'] + 'px)';
								if(!values['width'] && values['height'])
									dim = '(' + values['height'] + 'px,' + values['height'] + 'px)';
								if(values['width'] && values['height'])
									dim = '(' + values['width'] + 'px,' + values['height'] + 'px)';
								editor.insertText('[img' + dim + ']' + values['src'] + '[/img]');
							}
						}
					);
				}},
				...
				link: { txtExec: function(caller) {
					var editor = this;

					$.sceditor.command.get('link')._dropDown(
						editor,
						caller,
						function(values) {
							if(values['src'] && values['src'] !== "http://")
								editor.insertText('[url=' + values['src'] + ']' + values['desc'] + '[/url]');
						}
					);
				} },
				quote: { txtExec: function(caller) {
					var editor = this;
					
					$.sceditor.command.get('quote')._dropDown(
						editor,
						caller,
						function(author) {
							if(author)
								editor.insertText('[quote="' + author + '"]', '[/quote]');
							else
								editor.insertText('[quote]', '[/quote]');
						}
					);
				}},
...
// TODO:This should check for CSS white-space, should pass it in the function to reduce css lookups which are SLOW!
	...
	$.sceditor.plugins.bbcode.bbcodes = {
		...
		table: {
			...
			format: function(element, content) {
				var validAttributes = new Array('id', 'class', 'style', 'align', 'bgcolor', 'border', 'cellpadding', 'cellspacing', 'frame', 'rules', 'summary', 'width');
				var attributes = '';
				for(var i = 0; i < validAttributes.length; i++) {
					if(typeof(element.attr(validAttributes[i])) !== "undefined") {
						attributes +=  ' ' + validAttributes[i] + '="' + element.attr(validAttributes[i]) + '"';
					}
				}
				return attributes ? "[table" + attributes + "]" + content + "[/table]" : "[table]" + content + "[/table]";
			},
			html: function(token, attrs, content) {
				var attributes = '';
				for(var k in attrs) {
		...
		tr: {
			...
			quoteType: $.sceditor.BBCodeParser.QuoteType.always,
			format: function(element, content) {
				var validAttributes = new Array('id', 'class', 'style', 'align', 'bgcolor', 'char', 'charoff', 'valign');
				var attributes = '';
				for(var i = 0; i < validAttributes.length; i++) {
					if(typeof(element.attr(validAttributes[i])) !== "undefined") {
						attributes +=  ' ' + validAttributes[i] + '="' + element.attr(validAttributes[i]) + '"';
				...
				return attributes ? "[tr" + attributes + "]" + content + "[/tr]" : "[tr]" + content + "[/tr]";
			},
			html: function(token, attrs, content) {
				var attributes = '';
				for(var k in attrs) {
		...
		td: {
			...
			quoteType: $.sceditor.BBCodeParser.QuoteType.always,
			format: function(element, content) {
				var validAttributes = new Array('id', 'class', 'style', 'abbr', 'align', 'axis', 'bgcolor', 'char', 'charoff', 'colspan', 'headers', 'height', 'nowrap', 'rowspan', 'scope', 'valign', 'width');
				var attributes = '';
				for(var i = 0; i < validAttributes.length; i++) {
					if(typeof(element.attr(validAttributes[i])) !== "undefined") {
						attributes +=  ' ' + validAttributes[i] + '="' + element.attr(validAttributes[i]) + '"';
				...
				return attributes ? "[td" + attributes + "]" + content + "[/td]" : "[td]" + content + "[/td]";
			},
			html: function(token, attrs, content) {
				var attributes = '';
				for(var k in attrs) {
		...
		img: {
			...
			format: function($element, content) {
				...
				if((element.complete && (w || h)) || (w && h))
					attribs = "(" + $(element).width() + "px," + $(element).height() + "px)";
			...
			html: function(token, attrs, content) {
				...
				if(typeof attrs.defaultattr !== "undefined") {
					parts = attrs.defaultattr.split(/,/i);

					attribs = ' width="' + parts[0].substr(1) + '"' +
						' height="' + (parts.length === 2 ? parts[1].substr(0,parts[1].length-1) : parts[0].substr(1)) + '"';
				}
		...
		url: {
			...
			format: function(element, content) {
				...
				return '[url=' + $.sceditor.FAdecodeURI(url) + ']' + content + '[/url]';
			},
			html: function(token, attrs, content) {
		...
		quote: {
			...
			format: function(element, content) {
				...
				if(($cite && $cite.text()) || $elm.data("author")) {
			...
			html: function(token, attrs, content) {
				if(typeof attrs.defaultattr !== "undefined" && attrs.defaultattr !== '')
		...
		dailymotion: {
			...
			format: function(element, content) {
				if(!(element = element.attr('data-dailymotion-id')))
					return content;
				return '[dailymotion]' + element + '[/dailymotion]';
			},
			html: '<iframe width="560" height="315" src="http://www.dailymotion.com/embed/video/{0}?wmode=opaque" data-dailymotion-id="{0}" frameborder="0" allowfullscreen></iframe>'
		},
