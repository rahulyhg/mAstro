...
function sce (a, b) {
	...
	$(b + a).find('textarea').not('.one-cmt textarea, .non-sce, .non-me').each(function () {
		if (!$(this).next('.meditor-container').length) {
			if ($(this).is('.full-me')) {
				$(this).meditor({
					toolbar: [
...
function mtip(a, c, title, content) {
	...
	if (a && a.length) {
		...
		} else $(a).prepend('<div class="alerts alert-' + c + ' just-add"><a class="close" onclick="htip(\'just-add\')" data-dismiss="alert">\u00d7</a><strong>' + title + " </strong>" + content + "</div>");
...
//	var dv = '';
//	if (window.location.href.indexOf('.html/admincp') == -1)
	flatApp();
	validator();
}
...
function displayBoard (data) {
	data = data.split(/<!--BEGIN BOARD CONTENT-->|<!--END BOARD CONTENT-->/)[1];
	bStyle = data.split(/<!--\[bStyle\]|\[\/bStyle\]-->/)[1];
	$('.loading-screen').remove();
	$('body').addClass('fixed');
	$('#content').show().prepend(data).slideDown(40, function () {
		$board = $('.the-board');
		if (!$board.children('#content-board').length) $board.show().wrapInner('<div id="content-board"></div>');
		if (!$board.children('.board-close').length) $board.prepend('<a class="board-close" href="javascript:history.go(-1)"></a>');
		if (!$board.next('.the-board-fixed').length) $board.after('<div class="the-board-fixed"></div>').fadeIn(250);
		$board.attr('style', '').wrap('<div class="the-board-wrapper"><div class="the-board-inner"></div></div>');
		if (bStyle) $board.attr('style', bStyle);
//		$('.the-board, .the-board-fixed').wrapAll('<div class="the-board-wrapper"><div class="the-board-inner"></div></div>')
		if ($('#content-board').find('textarea').not('.one-cmt textarea, .non-sce, .non-me').length) 
			sce('content-board');
		refreshScripts();
	})
}
	...
	$('.bootstrap-validator-form').bootstrapValidator({
			...
			title: {
				...
				validators: {
					...
					stringLength: {
						min: 5,
						max: 200,
						message: 'This filed requires more than 4 and less than 200 characters long'
					}
				}
			},
			action: {
				...
				validators: {
					...
					stringLength: {
						min: 6,
						max: 300,
						message: 'This field requires more than 5 and less than 300 characters long'
					}
				}
			},
			content: {
				...
				validators: {
					...
					stringLength: {
						min: 11,
						message: 'This field requires more than 10 characters long'
					}
				}
			},
			date: {
		...
		if ($form.find('textarea').length) {
			$form.find('textarea').each(function () {
				if ($(this).next('.meditor-container').length) {
					val = $(this).next('.meditor-container').find('.meditor-iframe').html();
					$(this).val(val)
				}
			})
		}
...
//				if (alertsContent) $form.prepend('<div class="alerts alert-' + alertsType + '">' + alertsContent + '</div>');
//				if (alertsContent) mtip('form.just-sent', alertsType, '', alertsContent);
...
*/				$form.find('[type^="submit"]').attr('disabled', false);
				if ($form.hasClass('task-list')) {
					$form.slideUp(200).slideDown(200, function () {
						$(this).css('overflow', 'visible')
					}).css('overflow', 'visible');
				}
				if ($form.hasClass('generate-div')) {
					$('.preview-form .arrow-div');
					$('.preview-forms').html('<div class="spinner loading"><div></div><div></div><div></div></div>').load(window.location.href + ' .preview-forms > div');
				}
				if ($form.is('.acp-permission') || $form.is('.edit-channel-members')) $('.current-admins').load(window.location.href + ' .current-admins > div', function () {
						removeAdmin()
					});
				if ($form.hasClass('new-task-form')) $('.preview-list').load(window.location.href + ' .preview-list > div', function () {
						flatApp()
					});
				if ($form.hasClass('new-channel-quick')) $('.feed-channel').load(window.location.href + ' .feed-channel > div');
				if ($form.hasClass('form-new-faq')) $('.faq-list').load(window.location.href + ' .faq-list > div');
				if ($form.hasClass('form-new-journal')) $('.journal-list').load(window.location.href + ' .journal-list > div');
				if ($form.hasClass('rep-reg-info')) {
					var jv = $form.closest('.one-todo').attr('id');
					var jtype = $form.closest('.one-todo').children('label').attr('data-type');
					$('.rep-reg-list').load(window.location.href + ' .one-todo.alt-' + jtype + '-' + jv + ' .rep-reg-list > div');
				}
				if ($form.hasClass('reg-form')) $('.abox-list').load(window.location.href + ' .abox-list > div', function () {
						aboxList()
					});
				if ($form.hasClass('form-utsk') && $('body').find('.list-todo').length) {
					alert ('shit');
					$('.list-todo').load(window.location.href + ' .list-todo > div', function () {
						all()
					});
				}
				if ($form.hasClass('form-receive') && $('body').find('.list-todo').length) {
					$('.abox-list').load(window.location.href + ' .abox-list > div', function () {
						aboxList()
					});
				}
//				if ($form.hasClass('new-camp-form')) window.location.href += '&g=' + alertsDataID;
				if ($form.hasClass('new-camp-form')) {
					iid = 0;
					stepIn($form, 4);
				}
				if ($form.hasClass('edit-utask-form')) $('.list-todo').load(window.location.href + ' .list-todo > li', function () {
					flatApp(); lis()
				});
				if ($form.hasClass('edit-name-form')) window.location.href = MAIN_URL;
				if (window.location.url.indexOf('/new') !== -1) window.history.back();
			},
			error: function (data) {
				$form.addClass('just-sent').children('.spinner.loading-sending').remove();
				$form.children('*.visible.hide-to-load').show().removeClass('visible hide-to-load');
...
$(function () {
	refreshScripts();
/*	if ($('#content').find('textarea').not('.one-cmt textarea, .non-sce, .non-me').length) 
		sce('content');
	title = $('body').html().split(/<!--\[title\]|\[\/title\]-->/)[1];
*/});
