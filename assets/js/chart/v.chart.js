		...
		$.ajax({
			...
			success: function (data) {
				...
				$para.find('.product-vote-buttons > a').click(function () {
					...
					$.ajax({ url: MAIN_URL+'/report/'+rid+'?do='+act+'&v=window', type: 'post' })
				})
			}
		})
	}
...
//			$(this).attr('data-v', val);
	...
	$('.chart-report-content').on('scroll', function () {
		...
		$(this).find('.paragraph').each(function () {
			...
			tpp = document.getElementById(tocID).offsetTop + 40;
			btm = tpp + $paraDiv.height() - 160;
			if (scroll >= btm || scroll < tpp) $paraDiv.find('.product-vote').removeClass('fixed');
			else if (scroll >= tpp) {
				console.log($paraDiv.attr('id') + '~~~~' + scroll + '~~~~' + tpp + '~~~~' + btm);
				$paraDiv.find('.product-vote').addClass('fixed');
			}
		});
	});
	...
	$('#overview .chart-paragraph-content.des').css('max-height', 160).click(function () {
		if ($(this).css('max-height') == '100%') $(this).css('max-height', 160);
		else $(this).css('max-height', '100%')
	});
	$('.toggle-des').each(function () {
		if ($(this).next('.des').html().length) {
			$(this).append('<span class="fa fa-caret-down"></span>');
			$(this).click(function () {
				$(this).next('.des').toggle(200)
			})
		}
	});
	$('#dominant table').each(function () {
		$(this).find('tbody tr').hide();
		$(this).before('<div class="toggle-tbl fa fa-caret-down"></div>');
		$(this).prev('.toggle-tbl').andSelf().wrapAll('<div class="table-toggle"/>');
		$(this).prev('.toggle-tbl').click(function () {
			$tbl = $(this).next('table');
...
//		$this = $(this);
	...
	$('.chart-report-content .paragraph .report-switch').click(function() {
		...
		$div = $para.children('div.chart-paragraph-content');
		id = $(this).closest('.paragraph').attr('data-rid');
		$('.the-board').load(MAIN_URL+'/report/'+id+'?v=window&type=related', function () {
			popup();
